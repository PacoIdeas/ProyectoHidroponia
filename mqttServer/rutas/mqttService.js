    ...
    if (topic === t_camara_res) { // se recibe foto
     ...
     // console.log('lenght', message.length);
      ...
      try{
        ...
        const conn = await pool.getConnection(); //obtener la conexion
        const query = "INSERT INTO mediciones_ambientales (fecha_hora, temperatura, humedad) VALUES (?, ?, ?);"; //crea consulta



        const rows = await conn.query(query,[fecha_hora, valores[0], valores[1] ]);//ejecutar la consulta
